...
def pred_prob_for_mask(model, tokenizer, masked_sentence, target, ids = False):
    ...
    if ids == False:
        ...
        # print('Masked sentence:', tokenizer.decode(masked_sentence[1:-1]))
    ...
    output = model(input_ids) # This is a tuple consisting of (logits, hidden_states) or (logits), depending on the model.
    # print('Output format:', output.keys())
    logits = output['logits'] # logits is a tensor of shape (batch_size, sequence_length, vocab_size), so this corresponds to the only sentence in the batch.
    target_token_ids = tokenizer(target)['input_ids']
    # print(target, target_token_ids[1:-1])
    ...
    # print('Predicted probability for target:', predicted_prob_for_target)
...
def pll_scores(sent1, sent2, tokenizer, model):
    ...
    for i in range(1,num_shared_tokens-1):
        ground_truth1 = tokenizer.convert_ids_to_tokens([sent1_ids[0][shared_ind1[i]]])[0]
        ground_truth2 = tokenizer.convert_ids_to_tokens([sent2_ids[0][shared_ind2[i]]])[0]
