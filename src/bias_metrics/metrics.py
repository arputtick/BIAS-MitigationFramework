...
class WEAT:
    def __init__(self, model, enc = 'default'): # model is needed as it contains the method to get the vector of a word: get_vector(word)
        ...
        This is only important for BERT and GPT models, where the encoding can be associated to either
        the [CLS]/last ('default') token or targ./attr. ('token-level') token.
        '''
        self.model = model
        self.enc = enc
    ...
    def get_target_tokens(self, texts):
        ...
        if "." in texts[0]:
            pass
            if self.enc == 'token-level' and 'gpt' in self.model.model_path:
                matches = match_templates(texts, language = self.language, modeltype = 'gpt_pooling')
            else:
...
class LPBS:
    ...
    def p_tgt(self, template, target, attribute):
        ...
        sentence_masked = sentence.replace("TARGET", self.tokenizer.mask_token)
    ...
    def p_prior(self, template, target):
        ...
        '''
        sentence = template.replace("TARGET", target).replace("ATTRIBUTE", self.tokenizer.mask_token)
        sentence_masked = sentence.replace(target, self.tokenizer.mask_token)
        return pred_prob_for_mask(self.model, self.tokenizer, sentence_masked, target)
    ...
    def effect_size_log(self, target1, target2, attribute1, attribute2, templates):
        ...
        # attr1 = attribute1[0]
        # attr2 = attribute2[0]
        # targ1 = target1['singular'][0]
        # targ2 = target2['singular'][0]
        # df_1.to_csv(f'LPBS_{attr1}_{targ1}_{targ2}.csv')
        # df_2.to_csv(f'LPBS_{attr2}_{targ1}_{targ2}.csv')
        ...
        return (mean_attribute1 - mean_attribute2) / stdev , df_1, df_2, stdev
    ...
    def evaluate(self, dataset, calc_pval, number_permut, full_permut = False): # included the option to not calculate the p-value, as it takes a long time
        ...
        effect, df_1, df_2, stdev = self.effect_size_log(target1, target2, attribute1, attribute2, templates)
        print(f'effect size: {effect:.2f}')
        p_value = None
        if calc_pval:
            if full_permut:
                print('Too many permutations, defaulting to 10000')
                p_value = self.permutation_test(effect, df_1, df_2, stdev, num_permutations=10000)
            else:
                p_value = self.permutation_test(effect, df_1, df_2, stdev, num_permutations=number_permut)
    ...
    def permutation_test(self, observed_test_stat, df_1, df_2, stdev, num_permutations):
        '''
        Computes the one-sided p-value for LPBS. This differs from the original implementation, which computes a two-sided p-value.
        This also differs from WEAT, since partitions of atribute words are used instead of target words (as in Kurita).
        df_1 and df_2 are dataframes containing the bias scores of attribute1 amd attribute2, respectively.
        '''
        num_attr = len(df_1)
        print('calculating p-value, % of permutations done: ')
        count = 1
        for i in range(num_permutations):
            ...
            # Split the dataframe
            df_1 = df.iloc[:num_attr]
            df_2 = df.iloc[num_attr:]
            try:
                len(df_1) == len(df_2)
            except:
                # handle the exception here
                print('Error: df_1 and df_2 have different lengths.')
...
class CrowS_Pairs:
    ...
    def evaluate(self, dataset):
        ...
        df = dataset
        # df = dataset.iloc[:5] # for testing purposes
        print(df)
